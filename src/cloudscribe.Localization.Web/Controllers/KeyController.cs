...
namespace cloudscribe.Localization.Web
{
    public class KeyController : BaseController
    {
        ...
        public KeyController(ILocalizationDbContext context, ILogger<KeyController> logger, ITranslationService? translationService = null)
        {
        ...
        [HttpGet]
        public async Task<IActionResult> Index(KeyIndexModel model)
        {
        ...
        {
            var model = new KeyEditModel
            {
                Item = new Key() { DomainId = domainId ?? 0, MimeType = MediaTypeNames.Text.Plain }
            };
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            var model = new KeyEditModel() { Item = item };
        ...
        [HttpPost]
        public IActionResult Preview(int id, KeyEditModel model, string previewCulture)
        {
            return Content(model.Values.Single(v => v.Culture == previewCulture).Value ?? "(Empty)", MediaTypeNames.Text.Html);
        }

        [HttpPost]
        public async Task<IActionResult> AutoTranslate(int id, KeyEditModel model, CancellationToken cancellationToken)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> MarkAllReviewed(int id, KeyEditModel model)
        {
            ModelState.Clear();
        ...
        [HttpPost]
        public async Task<IActionResult> Submit(int id, KeyEditModel model)
        {
            ModelState.Clear();
            model.HasChanges = true;
        ...
        [HttpPost]
        public async Task<IActionResult> Save(int id, KeyEditModel model, bool apply = false)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    foreach (var value in model.Values.Where(v => !v.Reviewed && v.Value == null && v.Id != default)) context.LocalizeKeyValues.Remove(value);
                    ...
                    context.LocalizeKeys.Update(model.Item);
        ...
        [HttpPost]
        public async Task<IActionResult> Delete(int id, KeyEditModel model)
        {
            try
            {
                ...
                if (item != null)
                {
                    context.LocalizeKeys.Remove(item);
                    await context.SaveChangesAsync();
                }
                return Back(false);
            }
        ...
        private async Task<IActionResult> EditView(KeyEditModel model)
        {
